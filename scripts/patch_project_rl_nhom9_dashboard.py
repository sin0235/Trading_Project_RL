...
""",
        ...
        (
            ...
            "<div class=\"compare\" style=\"margin-top:14px\"><div class=\"cmp\"><span>Chênh hiện tại vs policy so sánh</span><strong class=\"${cls(currentVsCompare || 0)}\">${pct(currentVsCompare, 2)}</strong></div><div class=\"cmp\"><span>Chênh cuối kỳ vs policy so sánh</span><strong class=\"${cls(finalVsCompare || 0)}\">${pct(finalVsCompare, 2)}</strong></div><div class=\"cmp\"><span>Chênh giá trị cuối kỳ</span><strong class=\"${cls(finalValueGap || 0)}\">${money(finalValueGap)}</strong></div><div class=\"cmp\"><span>Số lệnh trong ngày</span><strong>${esc(f.trade_count || 0)}</strong></div><div class=\"cmp\"><span>Checkpoint đang xem</span><strong class=\"${cls((selected.summary || {}).final_return_pct || 0)}\">${pct((selected.summary || {}).final_return_pct, 2)}</strong></div><div class=\"cmp\"><span>Policy so sánh</span><strong class=\"${cls((compare.summary || {}).final_return_pct || 0)}\">${pct((compare.summary || {}).final_return_pct, 2)}</strong></div>${ddqCompare ? `<div class=\"cmp\"><span>Policy cố định</span><strong class=\"${cls((ddqCompare.summary || {}).final_return_pct || 0)}\">${pct((ddqCompare.summary || {}).final_return_pct, 2)}</strong></div><div class=\"cmp\"><span>Chênh cuối kỳ vs policy cố định</span><strong class=\"${cls(finalVsDdq || 0)}\">${pct(finalVsDdq, 2)}</strong></div>` : ''}</div>",
        ),
    ]
    ...
    out = replace_between(
        ...
        <section class="riskCharts">
          <div class="panel">
            ...
            <div class="legend"><span><i style="background:#20d6a4"></i>Checkpoint đang xem</span><span><i style="background:#ff7a88"></i>Policy so sánh</span>${ddqCompare ? '<span><i style="background:#e2e8f0"></i>Policy cố định</span>' : ''}<span><i style="background:#73b6ff"></i>Mua và giữ</span></div>
          ...
          <div class="panel">
            ...
            <div class="legend"><span><i style="background:#20d6a4"></i>Sharpe của checkpoint đang xem</span><span><i style="background:#ffd166"></i>Sortino của checkpoint đang xem</span><span><i style="background:#ff7a88"></i>Sharpe của policy so sánh</span>${ddqCompare ? '<span><i style=\"background:#e2e8f0\"></i>Sharpe của policy cố định</span>' : ''}</div>
